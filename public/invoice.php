...
$text = $input["text"];
$button = $input["button"];
$tg_token = "6544770916:AAEfmSPV6pBZPZn9j7ohgpBUQCY15lbdWEE";         //  Токен бота указать в этой стоке вместо слова token внутри кавычек
$tg_id = $input["chat_id"];
$bitrix = $input["bitrix"];
...
$text_message = $message.PHP_EOL.'Разом: '.$summUAH_itog.' UAH'.PHP_EOL.PHP_EOL.$text;
...
if ($tg_id != NULL) {
	...
	$sendTG["text"] = "Нове замовлення: № ".$file_count[$md5].PHP_EOL.$text_message;
    $sendTG["text"] = "Нове замовлення: № ".$file_count[$md5].PHP_EOL.$text_message.PHP_EOL;
